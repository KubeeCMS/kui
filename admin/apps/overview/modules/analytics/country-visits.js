  ...
  return {
    ...
    methods: {
      getData() {
        ...
        self.analytics = true;
        self.tableData = self.returnGAdata.countries.report.data;

        for (var i = 0; i < self.tableData.length; i++) {
          var item = self.tableData[i];
          countryData[item.name] = item.value;
        }
      ...
      getFlag(search) {
        var cCode = "";
        cCode = this.getKeyByValue(uip_country_list, search);

        if (cCode && cCode != "") {
          cCode = cCode.toLowerCase();
        }
        return "https://flagcdn.com/16x12/" + cCode + ".png";
      },
      ...
      '<div class="uip-padding-s uip-position-relative" :accountConnected="isGAconnected">\
        ...
        <template v-else>\
          ...
          <div v-if="overviewData.globalDataObject.loading != true && !overviewData.ui.editingMode  && isGAconnected" class="uip-overflow-auto">\
            <uip-country-chart v-if="cardOptions.hideMap != \'true\'" :translations="overviewData.translations" :cdata="countries" :dates="overviewData.dateRange"></uip-country-chart>\
            <!-- TABLE HEADERS -->\
                ...
                <div v-for="item in tableData" class="uip-margin-bottom-s">\
                    <div class="uip-flex uip-flex-center uip-margin-bottom-xs">\
                      <div class="uip-flex-grow uip-flex uip-flex-center uip-overflow-hidden uip-text-ellipsis uip-no-wrap uip-overflow-hidden uip-text-ellipsis uip-no-wrap">\
                        <img :src="getFlag(item.name)" alt="flag" @error="defaultImage" class="uip-w-20 uip-margin-right-xxs">\
            ...
            <form v-if="overviewData.ui.editingMode" class="uk-form-stacked" style="padding: var(--a2020-card-padding);">\
                <div class="">\
                    <div class="uip-text-bold uip-margin-bottom-xs">{{overviewData.translations.showmap}}</div>\
