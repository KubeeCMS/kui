...
class Admin_2020_settings {
	...
	public function start_front(){
		...
		add_action('wp_head', array( $this, 'load_custom_font_front'));
	...
	public function get_settings_options(){
		...
		if($activated) {
			...
			$temp['description'] = __("UIPress is activated. If you want to remove the licence click the remove licence button.",'admin2020');
	...
	* @since 1.4
	*/
	public function load_custom_font_front() {
		...
		if($font[0] && $font[1]){
			...
			echo '.a2020-admin-bar, .uk-navbar-nav > li > a, .uk-navbar-item, .uk-navbar-toggle, .admin2020_menu, #wpadminbar * {font-family: ' . $formattedfont . '}';
			echo '.uk-h1, .uk-h2,  .uk-h3, .uk-h4, .uk-h5, .uk-h6, .uk-heading-small, .uk-heading-medium, .uk-heading-large, .uk-heading-xlarge, .uk-heading-2xlarge {font-family: ' . $formattedfont . '}';
			echo '</style>';
		}
